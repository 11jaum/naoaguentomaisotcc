...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $username_psicologo = mysqli_real_escape_string($con, $_POST['username_psicologo']);
    ...
    if (empty($username_psicologo) || empty($nome_psicologo) || empty($cpf_psicologo) || empty($especialidade) || empty($crp) || empty($email) || empty($descricao) || empty($disponibilidade)) {
    ...
    $stmt = $con->prepare("INSERT INTO psicologos (username_psicologo, nome_psicologo, cpf_psicologo, especialidade, crp, disponibilidade, email, descricao) 
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssss", $username_psicologo, $nome_psicologo, $cpf_psicologo, $especialidade, $crp, $disponibilidade, $email, $descricao);
